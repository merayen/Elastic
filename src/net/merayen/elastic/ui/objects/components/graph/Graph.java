package net.merayen.elastic.ui.objects.components.graph;
...
public class Graph extends UIObject {
	...
	protected void onDraw() {
		...
		Point2D m = this.getAbsolutePosition(0, 0);
		f.moveTo(m.getX(), m.getY());

		for(Segment s : segments) {
			// Main line
			Point2D p1 = this.getAbsolutePosition((float)s.curve_a.x, (float)s.curve_a.y);
			Point2D p2 = this.getAbsolutePosition((float)s.curve_b.x, (float)s.curve_b.y);
			Point2D p3 = this.getAbsolutePosition((float)s.end.x, (float)s.end.y);
