					...
					public void loop(DataPacket[] port_packets, int[] packet_offsets, int offset, int sample_count) {
						...
						if(fac_connected) {
							...
							for(int i = 0; i < sample_count; i++)
								output[offset + i] =
									a[packet_offsets[0] + i]  * (0.5f + f[packet_offsets[2] + i] / 2) + // TODO retrieve the fac_value from fac-port, if connected
									b[packet_offsets[1] + i]  * (0.5f - f[packet_offsets[2] + i] / 2);
						} else {
							for(int i = 0; i < sample_count; i++)
								output[offset + i] =
				...
				pbi = new PortBufferIterator(ports, new PortBufferIterator.IteratorFunc() {
					...
					public void loop(DataPacket[] port_packets, int[] packet_offsets, int offset, int sample_count) {
						...
						if(fac_connected) {
							...
							for(int i = 0; i < sample_count; i++)
								output[offset + i] =
									a_or_b[packet_offsets[0] + i]  * (a_connected ? (0.5f + f[packet_offsets[1] + i] / 2) : (0.5f - f[packet_offsets[1] + i] / 2));
						} else {
							for(int i = 0; i < sample_count; i++)
								output[offset + i] =
